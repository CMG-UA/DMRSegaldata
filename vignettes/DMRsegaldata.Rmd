...
calculate_delta_beta <- function(cpg) {
    if (cpg %in% rownames(beta)) {
        cancer_mean <- mean(beta[cpg, cancer_samples], na.rm = TRUE)
        normal_mean <- mean(beta[cpg, normal_samples], na.rm = TRUE)
        return(cancer_mean - normal_mean)
    }
    NA
}

delta_beta <- sapply(rownames(dmps), calculate_delta_beta)
dmps$delta_beta <- delta_beta
# show only a subset
dmps_subset <- dmps[sample(rownames(dmps), min(1000, nrow(dmps))), ]
